...
async function getParentPath(specifier, url) {
  ...
  const matchedPackage = Object.keys(importers).find((external) => chunkQTRSKNOJ_cjs.isDependencyPartOfPackage(specifier, external));
  if (!matchedPackage) {
    return null;
  }
...
async function resolve(specifier, context, nextResolve) {
  ...
  if (stubbedExternals.length > 0) {
    const isStubbed = stubbedExternals.some((ext) => chunkQTRSKNOJ_cjs.isDependencyPartOfPackage(specifier, ext));
    if (isStubbed) {
