...
async function getInputOptions2(entryFile, platform, env, {
  ...
  const projectRoot = closestPkgJson ? path.dirname(chunkQTRSKNOJ_cjs.slash(closestPkgJson)) : chunkQTRSKNOJ_cjs.slash(process.cwd());
  const { workspaceMap, workspaceRoot } = await chunkITCCYSAA_cjs.getWorkspaceInformation({ mastraEntryFile: entryFile });
  const analyzeEntryResult = await chunkITCCYSAA_cjs.analyzeBundle(
    [entryFile],
    entryFile,
    {
  ...
  if (Array.isArray(inputOptions.plugins)) {
    ...
    inputOptions.plugins.push(chunkITCCYSAA_cjs.aliasHono());
    inputOptions.plugins.push(chunkBGHYAAUQ_cjs.nodeModulesExtensionResolver());
  }
  return inputOptions;
}
...
async function getServerOptions(entryFile, outputDir, logger) {
  const result = await chunk7OOWEFCB_cjs.extractMastraOption("server", entryFile, outputDir, logger);
  if (!result) {
    return null;
  }
...
//# sourceMappingURL=chunk-STAQTEMY.cjs.map
//# sourceMappingURL=chunk-STAQTEMY.cjs.map