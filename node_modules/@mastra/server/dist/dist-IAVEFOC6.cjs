              ...
              parts.push(
                part.image instanceof URL ? {
                  ...
                  inlineData: {
                    ...
                    data: chunkHERDIEC2_cjs.convertUint8ArrayToBase64(part.image)
                  }
                }
              );
              break;
...
var googleFailedResponseHandler = chunkHERDIEC2_cjs.createJsonErrorResponseHandler({
...
var GoogleGenerativeAILanguageModel = class {
  ...
  async getArgs({
    ...
    const warnings = [];
    const googleOptions = chunkHERDIEC2_cjs.parseProviderOptions({
      provider: "google",
  ...
  async doGenerate(options) {
    ...
    const mergedHeaders = chunkHERDIEC2_cjs.combineHeaders(
      await chunkHERDIEC2_cjs.resolve(this.config.headers),
      options.headers
    );
    const {
      responseHeaders,
      ...
      successfulResponseHandler: chunkHERDIEC2_cjs.createJsonResponseHandler(responseSchema),
  ...
  async doStream(options) {
    const { args, warnings } = await this.getArgs(options);
    const body = JSON.stringify(args);
    const headers = chunkHERDIEC2_cjs.combineHeaders(
      await chunkHERDIEC2_cjs.resolve(this.config.headers),
      options.headers
    );
    const { responseHeaders, value: response } = await chunkHERDIEC2_cjs.postJsonToApi({
      ...
      body: args,
      failedResponseHandler: googleFailedResponseHandler,
      successfulResponseHandler: chunkHERDIEC2_cjs.createEventSourceResponseHandler(chunkSchema),
      abortSignal: options.abortSignal,
      fetch: this.config.fetch
    });
    const { contents: rawPrompt, ...rawSettings } = args;
...
function createGoogleGenerativeAI(options = {}) {
  var _a;
  const baseURL = (_a = chunkHERDIEC2_cjs.withoutTrailingSlash(options.baseURL)) != null ? _a : "https://generativelanguage.googleapis.com/v1beta";
  const getHeaders = () => ({
    "x-goog-api-key": chunkHERDIEC2_cjs.loadApiKey({
  ...
  const createChatModel = (modelId, settings = {}) => {
    ...
    return new GoogleGenerativeAILanguageModel(modelId, settings, {
      ...
      baseURL,
      headers: getHeaders,
      generateId: (_a2 = options.generateId) != null ? _a2 : chunkHERDIEC2_cjs.generateId,
...
//# sourceMappingURL=dist-IAVEFOC6.cjs.map
//# sourceMappingURL=dist-IAVEFOC6.cjs.map