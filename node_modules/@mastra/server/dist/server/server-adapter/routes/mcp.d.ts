/**
 * MCP Routes
 *
 * Registry routes provide access to the MCP server registry and tools.
 * Transport routes handle the MCP protocol communication (HTTP and SSE).
 */
export declare const MCP_ROUTES: readonly [import(".").ServerRoute<{
    page?: number | undefined;
    perPage?: number | undefined;
    offset?: number | undefined;
    limit?: number | undefined;
}, {
    servers: {
        id: string;
        name: string;
        version_detail: {
            version: string;
            release_date: string;
            is_latest: boolean;
        };
    }[];
    total_count: number;
    next: string | null;
}, "json", import(".").RouteSchemas<undefined, import("zod").ZodObject<{
    page: import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>;
    perPage: import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>;
    offset: import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>;
    limit: import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>;
}, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
    servers: import("zod").ZodArray<import("zod").ZodObject<{
        id: import("zod").ZodString;
        name: import("zod").ZodString;
        version_detail: import("zod").ZodObject<{
            version: import("zod").ZodString;
            release_date: import("zod").ZodString;
            is_latest: import("zod").ZodBoolean;
        }, import("zod/v4/core").$strip>;
    }, import("zod/v4/core").$strip>>;
    total_count: import("zod").ZodNumber;
    next: import("zod").ZodNullable<import("zod").ZodString>;
}, import("zod/v4/core").$strip>>, "GET", "/mcp/v0/servers">, import(".").ServerRoute<import(".").InferParams<import("zod").ZodObject<{
    id: import("zod").ZodString;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    version: import("zod").ZodOptional<import("zod").ZodString>;
}, import("zod/v4/core").$strip>, undefined>, {
    id: string;
    name: string;
    version_detail: {
        version: string;
        release_date: string;
        is_latest: boolean;
    };
    description?: string | undefined;
    package_canonical?: string | undefined;
    packages?: unknown[] | undefined;
    remotes?: unknown[] | undefined;
}, "json", import(".").RouteSchemas<import("zod").ZodObject<{
    id: import("zod").ZodString;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    version: import("zod").ZodOptional<import("zod").ZodString>;
}, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
    id: import("zod").ZodString;
    name: import("zod").ZodString;
    description: import("zod").ZodOptional<import("zod").ZodString>;
    version_detail: import("zod").ZodObject<{
        version: import("zod").ZodString;
        release_date: import("zod").ZodString;
        is_latest: import("zod").ZodBoolean;
    }, import("zod/v4/core").$strip>;
    package_canonical: import("zod").ZodOptional<import("zod").ZodString>;
    packages: import("zod").ZodOptional<import("zod").ZodArray<import("zod").ZodUnknown>>;
    remotes: import("zod").ZodOptional<import("zod").ZodArray<import("zod").ZodUnknown>>;
}, import("zod/v4/core").$strip>>, "GET", "/mcp/v0/servers/:id">, import(".").ServerRoute<{
    serverId: string;
}, {
    tools: {
        name: string;
        inputSchema: unknown;
        description?: string | undefined;
        outputSchema?: unknown;
        toolType?: string | undefined;
    }[];
}, "json", import(".").RouteSchemas<import("zod").ZodObject<{
    serverId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, undefined, undefined, import("zod").ZodObject<{
    tools: import("zod").ZodArray<import("zod").ZodObject<{
        name: import("zod").ZodString;
        description: import("zod").ZodOptional<import("zod").ZodString>;
        inputSchema: import("zod").ZodUnknown;
        outputSchema: import("zod").ZodOptional<import("zod").ZodUnknown>;
        toolType: import("zod").ZodOptional<import("zod").ZodString>;
    }, import("zod/v4/core").$strip>>;
}, import("zod/v4/core").$strip>>, "GET", "/mcp/:serverId/tools">, import(".").ServerRoute<{
    serverId: string;
    toolId: string;
}, {
    name: string;
    inputSchema: unknown;
    description?: string | undefined;
    outputSchema?: unknown;
    toolType?: string | undefined;
}, "json", import(".").RouteSchemas<import("zod").ZodObject<{
    serverId: import("zod").ZodString;
    toolId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, undefined, undefined, import("zod").ZodObject<{
    name: import("zod").ZodString;
    description: import("zod").ZodOptional<import("zod").ZodString>;
    inputSchema: import("zod").ZodUnknown;
    outputSchema: import("zod").ZodOptional<import("zod").ZodUnknown>;
    toolType: import("zod").ZodOptional<import("zod").ZodString>;
}, import("zod/v4/core").$strip>>, "GET", "/mcp/:serverId/tools/:toolId">, import(".").ServerRoute<import(".").InferParams<import("zod").ZodObject<{
    serverId: import("zod").ZodString;
    toolId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
    data: import("zod").ZodOptional<import("zod").ZodUnknown>;
}, import("zod/v4/core").$strip>>, {
    result: unknown;
}, "json", import(".").RouteSchemas<import("zod").ZodObject<{
    serverId: import("zod").ZodString;
    toolId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
    data: import("zod").ZodOptional<import("zod").ZodUnknown>;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    result: import("zod").ZodUnknown;
}, import("zod/v4/core").$strip>>, "POST", "/mcp/:serverId/tools/:toolId/execute">, import(".").ServerRoute<{
    serverId: string;
}, unknown, "mcp-http", import(".").RouteSchemas<import("zod").ZodObject<{
    serverId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, undefined, undefined, undefined>, "ALL", "/mcp/:serverId/mcp">, import(".").ServerRoute<{
    serverId: string;
}, unknown, "mcp-sse", import(".").RouteSchemas<import("zod").ZodObject<{
    serverId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, undefined, undefined, undefined>, "ALL", "/mcp/:serverId/sse">, import(".").ServerRoute<{
    serverId: string;
}, unknown, "mcp-sse", import(".").RouteSchemas<import("zod").ZodObject<{
    serverId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, undefined, undefined, undefined>, "POST", "/mcp/:serverId/messages">];
//# sourceMappingURL=mcp.d.ts.map