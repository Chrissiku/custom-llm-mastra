...
var Bundler = class extends bundler.MastraBundler {
  ...
  async getUserBundlerOptions(mastraEntryFile, outputDirectory) {
    ...
    try {
      const bundlerOptions = await chunk7OOWEFCB_cjs.getBundlerOptions(mastraEntryFile, outputDirectory);
  ...
  async analyze(entry, mastraFile, outputDirectory) {
    return await chunkITCCYSAA_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async installDependencies(outputDirectory, rootDir = process.cwd()) {
    const deps = new chunkQJGW77MV_cjs.DepsService(rootDir);
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap, enableEsmShim, externals }) {
    const { workspaceRoot } = await chunkITCCYSAA_cjs.getWorkspaceInformation({ mastraEntryFile });
    ...
    for (const toolPath of toolsPaths) {
      ...
      for (const path$1 of expandedPaths) {
        if (await fsExtra__default.default.pathExists(path$1)) {
          const fileService = new chunkQJGW77MV_cjs.FileService();
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    try {
      ...
      analyzedBundleInfo = await chunkITCCYSAA_cjs.analyzeBundle(
...
//# sourceMappingURL=chunk-ESYPNSOC.cjs.map
//# sourceMappingURL=chunk-ESYPNSOC.cjs.map