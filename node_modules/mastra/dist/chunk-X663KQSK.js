...
var createMastraProject = async ({
  ...
  p4.intro(color.inverse(" Mastra Create "));
  const projectName = name ?? await p4.text({
  ...
  "include": [
    ...
    p4.outro("Project created successfully");
    console.info("");
    return { projectName, result };
  } catch (error) {
    s2.stop();
    const errorMessage = error instanceof Error ? error.message : "An unexpected error occurred";
    p4.cancel(`Project creation failed: ${errorMessage}`);
...
async function createFromTemplate(args) {
  ...
  if (args.template === true) {
    ...
    if (!selected) {
      p4.log.info("No template selected. Exiting.");
      return;
    }
    ...
    if (isGitHubUrl(args.template)) {
      ...
      if (!validation.isValid) {
        ...
        p4.log.error("This does not appear to be a valid Mastra project:");
        validation.errors.forEach((error) => p4.log.error(`  - ${error}`));
      ...
      if (!found) {
        p4.log.error(`Template "${args.template}" not found. Available templates:`);
        templates.forEach((t) => p4.log.info(`  - ${t.title} (use: ${t.slug.replace("template-", "")})`));
  ...
  if (!projectName) {
    ...
    const response = await p4.text({
      message: "What is your project name?",
      defaultValue: defaultName,
      placeholder: defaultName
    });
    if (p4.isCancel(response)) {
      p4.log.info("Project creation cancelled.");
      return;
    }
  ...
  if (!llmProvider) {
    const providerResponse = await p4.select({
      message: "Select a default provider:",
      options: LLM_PROVIDERS
    });
    if (p4.isCancel(providerResponse)) {
      p4.log.info("Project creation cancelled.");
      return;
    }
  ...
  let initGit = false;
  const gitConfirmResult = await p4.confirm({
    message: "Initialize a new git repository?",
    initialValue: true
  });
  if (!p4.isCancel(gitConfirmResult)) {
  ...
  try {
    ...
    p4.log.error(`Failed to create project from template: ${error instanceof Error ? error.message : "Unknown error"}`);
    throw error;
  }
}

export { create, createLogger, init, logger, package_default };
//# sourceMappingURL=chunk-X663KQSK.js.map
//# sourceMappingURL=chunk-X663KQSK.js.map